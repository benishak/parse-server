...
class Expression {
    ...
    static createCondition(comperator, value) {
    ...
    static getDynamoQueryFilter(query = {}) {
        let QueryFilter = {};
    ...
    static getProjectionExpression(keys, _params) {
        let projection = [];
    ...
    static getUpdateExpression(object, _params, original = {}) {
        ...
        Object.keys($del).forEach(path => {
            if ($del[path] instanceof Array) {
                let value = lodash_1._.get(original, path, []);
                if (value instanceof Array) {
                    $del[path].forEach(item => {
                        if (item.constructor == Object) {
                            lodash_1._.pull(value, lodash_1._.find(value, item));
                        }
                        else {
    ...
    createExpression(key, value, op, not = false, _all = false) {
        ...
        let exp = '';
        let _key = Expression.transformPath(this, key, value);
        let _vl = this._v;
        switch (op) {
            case 'begins_with':
    ...
    static transformPath(params, path, value = undefined) {
        if (!path) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (attributes.indexOf(_key) == -1) {
                ...
                if (helpers_1.$.getKey(params.ExpressionAttributeNames, keys[i].replace(/\[[0-9]+\]/g, '')) === null) {
                    let index = helpers_1.$.count(params.ExpressionAttributeNames, '#' + _k);
                    if (index > 0) {
                        _k = _k + '_' + index;
                    }
                ...
                if (value !== undefined) {
                    if (i == (keys.length - 1)) {
                        let index = helpers_1.$.count(params.ExpressionAttributeValues, ':' + _k);
                        _vl = ':' + _k + '_' + index;
                        params.ExpressionAttributeValues[_vl] = value;
                    }
                }
            }
            keys[i] = keys[i].replace(keys[i].replace(/\[[0-9]+\]/g, ''), helpers_1.$.getKey(params.ExpressionAttributeNames, keys[i].replace(/\[[0-9]+\]/g, '')));
        }
        params._v = _vl;
        return keys.join('.');
    }
